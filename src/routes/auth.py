...
async def signup(body: UserModel,
                 ...
                 request: Request, db: AsyncSession = Depends(get_db)) -> JSONResponse | dict:
    ...
    if body.email and await repository_users.get_user_by_email(body.email, db):
        raise HTTPException(
            status_code=409,
            detail="Podany e-mail jest już zajęty."
        )
    ...
    return {
        "user": new_user,
        "detail": (
            "Użytkownik utworzony. Jeżeli podałeś e-mail, sprawdź skrzynkę."
        )
    }
...
async def login(body: OAuth2PasswordRequestForm = Depends(),
                db: AsyncSession = Depends(get_db)) -> JSONResponse | dict:
...
async def refresh_token(
    ...
    username = auth_service.decode_token(token, expected_scope="refresh_token")
...
async def request_email(
    ...
    logger.info(f"Przygotowuję wysyłkę emaila do {user.email}")
    token = auth_service.create_token(
        subject=user.email,
        scope="email_confirm",
        expires_delta=3600 * 24
    ...
    logger.info("Dodaję zadanie wysyłki emaila do background tasks")
    background_tasks.add_task(
        email_service.send_email,
        email=user.email,
        subject="Potwierdź swój e-mail",
    ...
    logger.info("Zadanie wysyłki emaila dodane")
...
async def request_password_reset(
    ...
    if not user or not user.confirmed:
        return {
            "message": (
                "Nie znaleziono potwierdzonego e-maila powiązanego z kontem."
            )
        }
    ...
    return {
        "message": (
            ...
            "(jeśli e-mail istnieje i jest potwierdzony)."
        )
    }
...
async def reset_password(
    ...
    # Weryfikacja tokena resetu
    email = auth_service.decode_token(
        data.token,
        expected_scope="reset_password"
    )
    user = await repository_users.get_user_by_email(email, db)
    if not user:
        raise HTTPException(
